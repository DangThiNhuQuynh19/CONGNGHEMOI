...
include_once(__DIR__ . "/../../controller/cchuyenkhoa.php");
    ...
    <div class="chuyen-khoa-container">
        ...
        if (is_int($ds) && $ds == -1) {
            ...
            while ($row = $ds->fetch_assoc()) {
                echo '<div class="khoa">';
                echo '<img src="../../image/' . htmlspecialchars($row['img']) . '" alt="Ảnh chuyên khoa">';
                echo '<div class="khoa-content">';
                echo '<h3>' . htmlspecialchars($row['tenchuyenkhoa']) . '</h3>';
                echo '<a href="chitietchuyenkhoa.php?id=' . htmlspecialchars($row['machuyenkhoa']) . '" class="xem-them">Xem thêm</a>'; 
                echo '</div>';
                echo '</div>';
            }
        }
